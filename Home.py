...
kpis = [
    ...
    (c2, "📦 Tổng đơn hàng",    f"{ord_kpi['total_orders']:,}",          "thành công",    ""),
    ...
    (c5, "⏱️ Thời gian giao hàng TB",     f"{ship_kpi['avg_lead_time']:.1f} ngày", "từ khi đặt→giao",  ""),
]
...
nav_items = [
    (n1, "📊", "Doanh Thu", "Revenue, trend, top categories", "1_Doanh_thu.py"),
    (n2, "📦", "Tồn Kho", "Stock levels, capacity", "2_Tồn_kho.py"),
    (n3, "🚚", "Vận Chuyển", "Lead time, late rate", "3_Vận_chuyển.py"),
    (n4, "📈", "Dự Báo", "Forecasting (Prophet)", "4_Dự_báo.py"),
    (n5, "🤖", "Chatbot", "Hỏi đáp dữ liệu qua Gemini", "5_Chatbot.py"),
]

for col, icon, title, desc, file_name in nav_items:
    with col:
        # Sử dụng st.page_link để tạo nút bấm chuyển trang chính thống của Streamlit
        st.page_link(
            f"pages/{file_name}",
            label=f"{icon} {title}",
            help=desc,
